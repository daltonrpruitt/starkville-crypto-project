## Starkville Cryptocurrency Mining Facility Project Information

I wrote this page as a means to help people get up to speed on what is currently known about this project, both as to the basic facts and events so far, and as to the arguments being made on both sides (that I know of).

So if this is your first time running into this page, I humbly ask that you read this in it's entirety (including the pages shown in the sidebar).

I know it's a lot, but if you are interested enough in this project to click on a link someone sent you about it or that you ran into in the "wild" of the internet, I think it is worth your time to have as complete an understanding as possible.

And, if you know more about the project, its technicalities, or non-confidential specifics about the project that is **not** presented on this page, maybe you could take the time to [email me](mailto:techconnect.gtr.ms@gmail.com) and help me fill in the gaps of this page... 😬 👉️👈️ Please...?

⚠️ This site is also a work in progress, so there are places that could use more information, but I didn't know which were important, so it's sort of a skeleton in places. Just a warning.

The Facts: &nbsp; [Timeline](timeline) &nbsp; | &nbsp; [Cryptocurrency/Blockchain 101](crypto_101)

The Arguments: &nbsp; [Arguments in Favor](in-favor) &nbsp; | &nbsp; [Concerns and Positions Against](concerns)

My own personal biases: &nbsp; [My Personal Bias](my_bias)
